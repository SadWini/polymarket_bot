...
namespace poly {
    class BinanceFeed : public std::enable_shared_from_this<BinanceFeed>, public IFeedClient {
        net::io_context& ioc_;
        ...
        std::string path_ = "/ws/btcusdt@aggTrade";
    ...
    public:
        BinanceFeed(net::io_context& ioc) : ioc_(ioc),
        ...
        void stop() {
            ...
            ws_.async_close(websocket::close_code::normal,
                beast::bind_front_handler(&BinanceFeed::on_close,shared_from_this()));
        }
    private:
        ...
        void fail(beast::error_code ec, char const* what) {
            ...
            reconnect_timer_.async_wait(beast::bind_front_handler(&BinanceFeed::on_timer, shared_from_this()));
        }
        ...
        void process_item(const json& item) {
            if (!item.contains("e")) return;
            if (item["e"] == "aggTrade") {
                MarketEvent evt;
                evt.venue = Venue::BINANCE;
                evt.symbol = item.value("s", "unknown");
                try {
                    auto p_str = item.value("p", "0");
                    evt.price = std::stod(p_str);

                    auto s_str = item.value("q", "0");
                    evt.size = std::stod(s_str);
                    evt.timestamp_exch = item["T"];
                } catch (...) {
                ...
                bool is_maker = item.value("m", false);
                evt.side = is_maker ? Side::BUY : Side::SELL;
